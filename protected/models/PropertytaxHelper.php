...
class PropertytaxHelper extends CModel {
	...
	public function getDemand($id) {
		...
		//added for exsumptor on educess, minsamekittax, subcess1, subcess2
		$minsamekittax_exsumptor_amount = 0;
		$samekittax_exsumptor_amount = 0;
		$waterpttax_exsumptor_amount = 0;
		$educess_exsumptor_amount = 0;
		$subcess1_exsumptor_amount = 0;
		$subcess2_exsumptor_amount = 0;
		foreach (Ptexsumptor::model()->findAll() as $ptexsumptor) {
			if (in_array($ptexsumptor->idptexsumptor, $idptexsumptors)) {
				$minsamekittax_exsumptor_amount += round($minsamekittax * $ptexsumptor->rebate / 100);
				$samekittax_exsumptor_amount += round($samekittax * $ptexsumptor->rebate / 100);
				$waterpttax_exsumptor_amount += round($waterpttax * $ptexsumptor->rebate / 100);
				$educess_exsumptor_amount += round($educess * $ptexsumptor->rebate / 100);
				$subcess1_exsumptor_amount += round($subcess1 * $ptexsumptor->rebate / 100);
				$subcess2_exsumptor_amount += round($subcess2 * $ptexsumptor->rebate / 100);
			}
		}
		//minsamekittax, educess, subcess1, subcess2 after deducting exsumptor
		$minsamekittax -= $minsamekittax_exsumptor_amount;
		$samekittax -= $samekittax_exsumptor_amount;
		$waterpttax -= $waterpttax_exsumptor_amount;
		$educess -= $educess_exsumptor_amount;
		$subcess1 -= $subcess1_exsumptor_amount;
		$subcess2 -= $subcess2_exsumptor_amount;
