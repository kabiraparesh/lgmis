...
class RprateController extends Controller
{
	/**
...
//        public $layout = '//layouts/eastsouthwest';
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Rprate']))
		{
			$model->attributes=$_POST['Rprate'];
                        $status = 'Failure';
                        if ($model->save()) {
                            $status = 'Success';
                        }
	...
	{
		$dataProvider=new CActiveDataProvider('Rprate');
	...
	{
		$model=new Rprate('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Rprate']))
			$model->attributes=$_GET['Rprate'];
	...
	{
		$model=Rprate::model()->findByPk((int)$id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='rprate-form')
		{
        ...
        public function actionJsonmessage() {
            ...
            if(isset ($_REQUEST['cid'])){
                $model=Rprate::model()->findByPk($_REQUEST['cid']);
                if($model){
