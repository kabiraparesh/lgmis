...
class RptenantController extends Controller
{
	/**
...
//        public $layout = '//layouts/eastsouthwest';
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Rptenant']))
		{
			$model->attributes=$_POST['Rptenant'];
                        $status = 'Failure';
                        if ($model->save()) {
                            $status = 'Success';
                        }
                    ...
                    if (isset($_REQUEST['isAjaxRequest']) && $_REQUEST['isAjaxRequest'] == 1) {
                        ...
                        Yii::app()->clientscript->scriptMap['jquery-ui.min.js'] = false;
                          ...
                          if (isset($_REQUEST['idparent'])) {
                            ...
                            $model->idrpshop = $_REQUEST['idparent'];
                        }  
	...
	{
		$dataProvider=new CActiveDataProvider('Rptenant');
	...
	{
		$model=new Rptenant('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Rptenant']))
			$model->attributes=$_GET['Rptenant'];
	...
	{
		$model=Rptenant::model()->findByPk((int)$id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='rptenant-form')
		{
        ...
        public function actionJsonmessage() {
            ...
            if(isset ($_REQUEST['cid'])){
                $model=Rptenant::model()->findByPk($_REQUEST['cid']);
                if($model){
