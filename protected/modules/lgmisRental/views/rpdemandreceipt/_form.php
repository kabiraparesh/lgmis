...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
		<?php echo $form->labelEx($model,'idrpdemand'); ?>		
                <?php echo $form->textField($model,'idrpdemand',array('size'=>10,'maxlength'=>10,'ajax'=>array('type'=>'POST', 'dataType'=>'json', 'data'=>array('cid'=>'js:$("#rpdemandreceipt-form").find(\'#Rpdemandreceipt_idrpdemand\').val()',), 'url'=>CController::createUrl('rpdemand/jsonmessage'), 'success'=>'function(data){$(\'#' . ucfirst(CHtml::activeId($model,'idrpdemand')) . '_msg\').text(data.message);}'))); ?>
                <span id="<?php echo ucfirst(CHtml::activeId($model,'idrpdemand')); ?>_msg">
                    <?php echo !isset($model->idrpdemand0->idrpshop) || $model->isNewRecord ? "-" : $model->idrpdemand0->idrpshop; ?>
                </span>
                <?php
                    $imghtml = CHtml::image(Yii::app()->request->baseUrl . '/images/tooltip.png');
                    echo CHtml::ajaxLink(
                            $imghtml, 
                            array('rpdemand/popupview'), // the URL for the AJAX request. If empty, it is assumed to be the current URL.
                            array(
                                "data" => array(
                                    "isAjaxRequest" => 1,
                                ),
                                'success' => 'function(data){
                                        window.cid = "rpdemandreceipt";
                                        window.eid = "Rpdemandreceipt_idrpdemand";
                                        window.url = "'. CController::createUrl('rpdemand/jsonmessage').'";
                                        $("#dialog-popup-select").html(data);
                                        $("#dialog-popup-select").dialog("option", "title", "'. Yii::t('application', 'Rpdemands') .'");
                                        $("#dialog-popup-select").dialog("open"); 
