...
class LprateController extends Controller
{
	/**
...
//        public $layout = '//layouts/eastsouthwest';
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Lprate']))
		{
			$model->attributes=$_POST['Lprate'];
                        $status = 'Failure';
                        if ($model->save()) {
                            $status = 'Success';
                        }
	...
	{
		$dataProvider=new CActiveDataProvider('Lprate');
	...
	{
		$model=new Lprate('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Lprate']))
			$model->attributes=$_GET['Lprate'];
	...
	{
		$model=Lprate::model()->findByPk((int)$id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='lprate-form')
		{
        ...
        public function actionJsonmessage() {
            ...
            if(isset ($_REQUEST['cid'])){
                $model=Lprate::model()->findByPk($_REQUEST['cid']);
                if($model){
