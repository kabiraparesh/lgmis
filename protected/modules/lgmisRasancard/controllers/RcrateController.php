...
class RcrateController extends Controller
{
	/**
...
//        public $layout = '//layouts/eastsouthwest';
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Rcrate']))
		{
			$model->attributes=$_POST['Rcrate'];
                        $status = 'Failure';
                        if ($model->save()) {
                            $status = 'Success';
                        }
	...
	{
		$dataProvider=new CActiveDataProvider('Rcrate');
	...
	{
		$model=new Rcrate('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Rcrate']))
			$model->attributes=$_GET['Rcrate'];
	...
	{
		$model=Rcrate::model()->findByPk((int)$id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='rcrate-form')
		{
        ...
        public function actionJsonmessage() {
            ...
            if(isset ($_REQUEST['cid'])){
                $model=Rcrate::model()->findByPk($_REQUEST['cid']);
                if($model){
