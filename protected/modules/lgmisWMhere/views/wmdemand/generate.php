...
<?php 
	$this->setPageTitle(LgmisWMModule::t('Generate Wmdemand'));
...
$count = Wmdemand::model()->count($criteria);
...
if($period<12){
	$this->widget('zii.widgets.jui.CJuiButton', array(
			'buttonType'=>'button',
			'name'=>'btnClick',
			'caption'=>LgmisWMModule::t('Generate Demand for Month: {month}', array('{month}' => Yii::app()->getLocale()->getMonthName($periodCaption+1))), 
			'options'=>array('icons'=>'js:{primary:"ui-icon-newwin"}'),
			'onclick'=>'js:function(){ generate("'. Yii::app()->createUrl("lgmisWM/wmdemand/generateAll", array('create'=>1)) .'");}',
	));
}
...
//             'height' => $this->dlgHeight,
            ...
            'modal' => true,
            'buttons' => array(
                LgmisWMModule::t('Cancel') => 'js:function(){ location.reload(true); $(this).dialog("close").destroy();}',
            ),
        ),
        'cssFile' => false,
        'scriptFile' => false,
