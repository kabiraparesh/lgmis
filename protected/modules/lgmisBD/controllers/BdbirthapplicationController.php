...
class BdbirthapplicationController extends Controller
{
	/**
...
//        public $layout = '//layouts/eastsouthwest';
	...
	{
		$model=new Bdbirthapplication;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['Bdbirthapplication']))
		{
			$model->attributes=$_POST['Bdbirthapplication'];
                        $status = 'Failure';
                        if ($model->save()) {
                            $status = 'Success';
                        }
                    ...
                    if (isset($_REQUEST['isAjaxRequest']) && $_REQUEST['isAjaxRequest'] == 1) {
                        ...
                        if (isset($_REQUEST['idparent'])) {
                            ...
                            $model->idbdbirthinformer = $_REQUEST['idparent'];
                        }  
	...
	{
		$dataProvider=new CActiveDataProvider('Bdbirthapplication');
	...
	{
		$model=new Bdbirthapplication('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Bdbirthapplication']))
			$model->attributes=$_GET['Bdbirthapplication'];
	...
	{
		$model=Bdbirthapplication::model()->findByPk((int)$id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='bdbirthapplication-form')
		{
        ...
        public function actionJsonmessage() {
            ...
            if(isset ($_REQUEST['cid'])){
                $model=Bdbirthapplication::model()->findByPk($_REQUEST['cid']);
                if($model){
                    $jsonmessage['message'] = $model->idbdbirthinformer ;
                }
            }
