		...
		if(isset($_POST['<?php echo $this->modelClass; ?>']))
		{
			...
			$model->attributes=$_POST['<?php echo $this->modelClass; ?>'];
                        <?php foreach ($fkeys as $key=>$value): ?>$criteria=new CDbCriteria( array(
                            'condition' => '<?php echo $value[1] ?> = :<?php echo $value[1] ?>',
                            'params'    => array(':<?php echo $value[1] ?>' => $_REQUEST['<?php echo $value[1] ?>'])
                        ) );            
                        $<?php echo strtolower($key); ?>=<?php echo $key; ?>::model()->find($criteria);
                        if(!isset($<?php echo strtolower($key); ?>)){
                            echo CJSON::encode(array(
                                'status' => $status,
                            ));
                            exit;                            
                        }
                        $model-><?php echo $value[0] ?> = $<?php echo strtolower($key); ?>-><?php echo $value[0] ?>;
