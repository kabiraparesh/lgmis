...
//        public $layout = '//layouts/eastsouthwest';
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				...
				'users'=>array('admin', 'user', 'user1', 'user2'),
			),
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Cczone']))
		{
			$model->attributes=$_POST['Cczone'];
                        $status = 'Failure';
                        $message = Yii::t('application', Yii::t('application', 'An error occurred while the form was being submitted.<br/>Please check your form data.'));
                        if ($model->save()) {
                            $status = 'Success';
                            $message = Yii::t('application', 'Form submitted successfully.');                        
                        }
	...
	{
		$dataProvider=new CActiveDataProvider('Cczone');
	...
	{
		$model=new Cczone('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Cczone']))
			$model->attributes=$_GET['Cczone'];
	...
	{
		$model=Cczone::model()->findByPk((int)$id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='cczone-form')
		{
        ...
        public function actionJsonmessage() {
            ...
            if(isset ($_REQUEST['cid'])){
                $model=Cczone::model()->findByPk($_REQUEST['cid']);
                if($model){
                    $jsonmessage['message'] = $model->zonename ;
                }
            }
