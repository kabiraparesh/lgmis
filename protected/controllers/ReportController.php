...
//             );
    ...
    /*
	Check for oldbalance > 0
    */    
    public function hasOldbalance($demand){
    	$isOldbalance = false;
    	foreach ($demand['data'] as $row){
    		if($row[1] > 1){
    			$isOldbalance = true;
    			break;
    		}
    ...
    public function actionViewBills($id, $type=0) {
        if ($id == 0) {
        ...
        foreach ($ptmasters as $ptmaster) {
            ...
            //Check whether any oldbalance should have non zero
            if($type==1){
            	if(!$this->hasOldbalance($demand)){
            		continue;
            	}
            }
...
//        }
    ...
    public function isNewProperty($ptmaster){
    	$params = array();
    ...
    public function actionReport($id, $report = 1, $type=0) {
        $criteria = new CDbCriteria(array(
                    'condition' => 'idccward = :idccward',
                    'params' => array(':idccward' => $id),
                    'order' => 'ledgerno'
        ...
        $demandsdetails = array();
        foreach ($ptmasters as $ptmaster) {
        	if($report == 6 && !$this->isNewProperty($ptmaster)){
        		continue;
        	}
            $idccward = $ptmaster->idccward;
        	$demand = $this->getDemand($ptmaster->idptmaster);
            ...
            $idccward = $ptmaster->idccward;
            $demandsdetails[] = array('ptmaster' => $ptmaster, 'demand' => $demand);
        }
...
public function actionReport5() {
    ...
    foreach ($ccwards as $ccward) {
        ...
        $propertytax_accounts = 0;
        $minsamekittax_accounts = 0;
        ...
        $wards[] = array(
                ...
                'propertytax_accounts' => $propertytax_accounts,
                'minsamekittax_accounts' => $minsamekittax_accounts,
...
public function actionReport5_2() {
    $ccwards = Ccward::model()->findAll();
    $wards = array();

    $grandPropertytax = 0;

    $grandMinsamekittax = 0;
    $grandSamekittax = 0;
    ...
    $grand_propertytax_accounts = 0;
    $grand_minsamekittax_accounts = 0;
    ...
    foreach ($ccwards as $ccward) {
        ...
        foreach ($ptmasters as $ptmaster) {
            ...
            $totalPropertytax += $demand['data'][0][3];

            $totalEducess += $demand['data'][3][3];
            $totalSubcess1 += $demand['data'][4][3];
            $totalSubcess2 += $demand['data'][5][3];

            if($demand['data'][0][3] > 0){
                $propertytax_accounts++;
                $grand_propertytax_accounts++;
            }
            else{
                if($demand['data'][1][3] > 0){
                    $totalMinsamekittax += $demand['data'][1][3];
                    $totalSamekittax += $demand['data'][2][3];
                    $minsamekittax_accounts++;
                    $grand_minsamekittax_accounts++;
                }
            }
        ...
        $grandPropertytax += $totalPropertytax;

        $grandMinsamekittax += $totalMinsamekittax;
        $grandSamekittax += $totalSamekittax;

        $grandEducess += $totalEducess;

        $grandSubcess1 += $totalSubcess1;
        $grandSubcess2 += $totalSubcess2;

        //                echo $paidamt; exit;
        $wards[] = array(
                'idccward' => $ccward->idccward,
                'totalPropertytax' => $totalPropertytax,
                'totalMinsamekittax' => $totalMinsamekittax,
                'totalSamekittax' => $totalSamekittax,
                'totalEducess' => $totalEducess,
                'totalSubcess1' => $totalSubcess1,
                'totalSubcess2' => $totalSubcess2,
                'propertytax_accounts' => $propertytax_accounts,
                'minsamekittax_accounts' => $minsamekittax_accounts,                        
                'total' => $totalPropertytax + $totalMinsamekittax + $totalSamekittax + $totalEducess + $totalSubcess1 + $totalSubcess2,
        );
    }

    $grandTotal = $grandPropertytax + $grandMinsamekittax + $grandSamekittax + $grandEducess + $grandSubcess1 + $grandSubcess2;
    echo $this->renderPartial('/report/report5_2',
            array(
                    'wards' => $wards,
                    'grandPropertytax' => $grandPropertytax,
                    'grandMinsamekittax' => $grandMinsamekittax,
                    'grandSamekittax' => $grandSamekittax,
                    'grandEducess' => $grandEducess,
                    'grandSubcess1' => $grandSubcess1,
                    'grandSubcess2' => $grandSubcess2,
                    'grand_propertytax_accounts' => $grand_propertytax_accounts,
                    'grand_minsamekittax_accounts' => $grand_minsamekittax_accounts,
                    'grandTotal' => $grandTotal,
            ),
            true, true);
}
    public function getDemandCollection($ptmaster) {
    	$data = array();
    	...
    	$criteria = new CDbCriteria(array(
    			'with' => array(
    					'idptmaster0' => array(
    							'condition' => 'idptmaster0.idptmaster =:idptmaster',
    							'params' => array(':idptmaster' => $ptmaster->idptmaster)
    					),
    			),
    			'condition' => 'idccfyear = :idccfyear',
    			'params' => array(':idccfyear' => Yii::app()->session['ccfyear']->idccfyear),
    	...
    	if (isset($pttransaction)) {
    		$currentpropertytax = $pttransaction->propertytax + $pttransaction->minsamekittax + $pttransaction->samekittax + $pttransaction->educess + $pttransaction->subcess1 + $pttransaction->subcess2;
    		$previouspropertytax = $pttransaction->oldpropertytax + $pttransaction->oldminsamekittax + $pttransaction->oldsamekittax + $pttransaction->oldeducess + $pttransaction->oldsubcess1 + $pttransaction->oldsubcess2 - $pttransaction->oldpttaxdiscount + $pttransaction->oldpttaxsurcharge;
    		...
    		$fddemandreceipts = Fddemandreceipt::model()->findAll($criteria);
//     		$fddemandreceiptsArr = $fddemandreceipts; 
    		foreach ($fddemandreceipts as $fddemandreceipt) {
    			$oldamountpaid += $fddemandreceipt->amountpaid;
    			$oldfddemandreceipts[] = $fddemandreceipt;
    			...
    			if (isset($fddemandreceipt->details)) {
    				...
    				if($fddemandreceipt->receipttype == 0){
    					$currenttaxpaid += $array["details-inputgrid[0][amount]"] + $array["details-inputgrid[0][discount]"];
    					$currenttaxpaid += $array["details-inputgrid[1][amount]"] + $array["details-inputgrid[1][discount]"];
    					$currenttaxpaid += $array["details-inputgrid[2][amount]"] + $array["details-inputgrid[2][discount]"];
    					$currenttaxpaid += $array["details-inputgrid[3][amount]"] + $array["details-inputgrid[3][discount]"];
    
    					$currenttaxpaid += $array["details-inputgrid[4][amount]"] + $array["details-inputgrid[4][discount]"];
    					$currenttaxpaid += $array["details-inputgrid[5][amount]"] + $array["details-inputgrid[5][discount]"];
    
    					$currenttaxpaid += $array["details-inputgrid[6][amount]"] + $array["details-inputgrid[6][discount]"];
    					$currenttaxpaid += $array["details-inputgrid[7][amount]"] + $array["details-inputgrid[7][discount]"];
    					$currenttaxpaid += $array["details-inputgrid[8][amount]"] + $array["details-inputgrid[8][discount]"];
    					$currenttaxpaid += $array["details-inputgrid[9][amount]"] + $array["details-inputgrid[9][discount]"];
    				}
    				else{
    					$previoustaxpaid += $array["details-inputgrid[0][amount]"] + $array["details-inputgrid[0][discount]"];
    					$previoustaxpaid += $array["details-inputgrid[1][amount]"] + $array["details-inputgrid[1][discount]"];
    					$previoustaxpaid += $array["details-inputgrid[2][amount]"] + $array["details-inputgrid[2][discount]"];
    					$previoustaxpaid += $array["details-inputgrid[3][amount]"] + $array["details-inputgrid[3][discount]"];
    
    					$previoustaxpaid += $array["details-inputgrid[4][amount]"] + $array["details-inputgrid[4][discount]"];
    					$previoustaxpaid += $array["details-inputgrid[5][amount]"] + $array["details-inputgrid[5][discount]"];
    
    					$previoustaxpaid += $array["details-inputgrid[6][amount]"] + $array["details-inputgrid[6][discount]"];
    					$previoustaxpaid += $array["details-inputgrid[7][amount]"] + $array["details-inputgrid[7][discount]"];
    					$previoustaxpaid += $array["details-inputgrid[8][amount]"] + $array["details-inputgrid[8][discount]"];
    					$previoustaxpaid += $array["details-inputgrid[9][amount]"] + $array["details-inputgrid[9][discount]"];
    				}
    				$data[] = array('ptmaster' => $ptmaster, 'fddemandreceipt'=>$fddemandreceipt, 'previoustaxpaid' => $previoustaxpaid, 'currenttaxpaid' => $currenttaxpaid, 'currentpropertytax' => $currentpropertytax, 'previouspropertytax' => $previouspropertytax);   				
      			}
    		}    
    	    if(count($data) == 0){
//     			$data[] = array('ptmaster' => $ptmaster, 'fddemandreceipt'=>NULL, 'previoustaxpaid' => 0, 'currenttaxpaid' => 0, 'currentpropertytax' => 0, 'previouspropertytax' => 0);    		
    			$data[] = array('ptmaster' => $ptmaster, 'fddemandreceipt'=>NULL, 'previoustaxpaid' => 0, 'currenttaxpaid' => 0, 'currentpropertytax' => $currentpropertytax, 'previouspropertytax' => $previouspropertytax);   				
    	    }
    	}
    	//if(count($data) == 0) means, there are now transaction against this demand... so add blank single row...
    	return $data;
//     	return array('data' => $data, 'pttransaction' => $pttransaction);
    }
...
/*
    public function getDemandCollection($id) {
    	$data = array();
    	...
    	$oldpropertytaxpaid = 0;
    	$oldservicetaxpaid = 0;
    	$oldminsamekittaxpaid = 0;
    	$oldsamekittaxpaid = 0;
    	$oldwaterpttaxpaid = 0;
    	$oldeducesspaid = 0;
    	$oldsubcess1paid = 0;
    	$oldsubcess2paid = 0;
    	$oldpttaxdiscountpaid = 0;
    	$oldpttaxsurchargepaid = 0;
    	$oldamountpaid = 0;
    	$olddiscountpaid = 0;
...
//     				$waterpttaxpaid += $array["details-inputgrid[9][amount]"] + $array["details-inputgrid[9][discount]"];
    				...
    				if($fddemandreceipt->receipttype == 0){
    					...
    					$oldpropertytaxpaid += $array["details-inputgrid[0][amount]"] + $array["details-inputgrid[0][discount]"];
    					$oldminsamekittaxpaid += $array["details-inputgrid[1][amount]"] + $array["details-inputgrid[1][discount]"];
    					$oldsamekittaxpaid += $array["details-inputgrid[2][amount]"] + $array["details-inputgrid[2][discount]"];
    					$oldeducesspaid += $array["details-inputgrid[3][amount]"] + $array["details-inputgrid[3][discount]"];
    						
    					$oldsubcess1paid += $array["details-inputgrid[4][amount]"] + $array["details-inputgrid[4][discount]"];
    					$oldsubcess2paid += $array["details-inputgrid[5][amount]"] + $array["details-inputgrid[5][discount]"];
    						
    					$oldpttaxdiscountpaid += $array["details-inputgrid[6][amount]"] + $array["details-inputgrid[6][discount]"];
    					$oldpttaxsurchargepaid += $array["details-inputgrid[7][amount]"] + $array["details-inputgrid[7][discount]"];
    					$oldservicetaxpaid += $array["details-inputgrid[8][amount]"] + $array["details-inputgrid[8][discount]"];
    					$oldwaterpttaxpaid += $array["details-inputgrid[9][amount]"] + $array["details-inputgrid[9][discount]"];
    				}
    		...
    		$data[] = array(
    				...
    				$pttransaction->oldpropertytax + $pttransaction->propertytax,
    				isset($propertytaxpaid) ? $propertytaxpaid : 0,
    				isset($oldpropertytaxpaid) ? $oldpropertytaxpaid : 0,
    				0,
    				0,
    				0,
    		);
