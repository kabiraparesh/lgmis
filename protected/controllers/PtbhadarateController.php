	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('popupview', 'admin','delete', 'generateBhadarate'),
				'users'=>array('admin'),
			),
        ...
        public function actionGenerateBhadarate($generate=false){
            ...
            $jsonmessage = array();            
            $status = 'Success';
            $message = Yii::t('application', 'Bhadarate will generating for financial year {fyear}. Are you sure?', array('{fyear}'=>Yii::app()->session['ccfyear']->fyear));
            if($generate){
                //delete any old entries...
                Ptbhadarate::model()->deleteAll($criteria);
                //add new entries
                $sql = "INSERT INTO {{ptbhadarate}} (idptpropertyon, idptrange, idccfyear)
                        SELECT idptpropertyon, idptrange, 1 as idccfyear FROM {{ptpropertyon}}, {{ptrange}} order by idptpropertyon";
                $connection=Yii::app()->db;
                $command=$connection->createCommand($sql);
                $rowCount=$command->execute();                
            }
            else{
                $count = Ptbhadarate::model()->count($criteria);
            }
            if($count > 0){
                $status = 'Exists';
                $message = Yii::t('application', 'Bhadarate already generated for financial year {fyear}. These items will be permanently deleted and cannot be recovered. Are you sure?', array('{fyear}'=>Yii::app()->session['ccfyear']->fyear));
            }
